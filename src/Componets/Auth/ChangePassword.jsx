...
export function ChangePassword() {
  ...
  const { SuccessMsg, ErrorMsg, WarningMsg } = UsePopUp();
  const [loading, setloading] = useState(false);
  const [data, setdata] = useState({ Password: "", RepPassword: "" });
  ...
  const LoginOTPhandler = async () => {
    try {
      if (data.Password !== data.RepPassword) {
        return WarningMsg("Password must be Same");
      }
      console.log(data.Password);

      const res = await axios.put(
        `${import.meta.env.VITE_API_URL}Users/PasswordUpdate`,
        {
          Email,
      ...
      console.log(res);
      if (res?.data?.Success) {
        SuccessMsg("Password is Chnage Successfully");
        navigate("/");
        setloading(false);
      }
      setloading(false);
          ...
          <div className="mb-4 ">
            <div className="flex items-center p-3 bg-gray-50 dark:bg-white/50 rounded-md">
              ...
              <input
                ...
                className="bg-transparent w-full font-serif  placeholder-gray-700 text-black focus:outline-none"
